...
## 三种ocr的区别
|         | 介绍                                                         | 劣势                                                         | 价格          | 识别准确度                   | 识别速度 | 竖排翻译 | 贴字翻译 |
| :------ | :----------------------------------------------------------- | :----------------------------------------------------------- | :------------ | :--------------------------- | :------- | :------- | :------- |
| 本地ocr | 在本地电脑上运行文字识别环境，使用百度飞桨开源的文字识别模型，识别精度上不会很高 | 使用本地电脑cpu，内存进行运算，低端电脑可能部署困难或无法运行 | `免费`          | 一般 | 取决于用户计算机性能 | ✔️ | ✔️ |
| 百度ocr | 百度官方的ocr接口，文字识别精度，响应速度都很优秀，但是`贵`  | 价格昂贵                                                     | 每`1w次/50元` | 高 | 一般在0.8s左右，非常稳定 | ✔️ | ❌ |
| 在线ocr | 我们团队内部以合作模式部署的在线ocr平台，文字识别模型使用了我们独立训练的模型，识别精度与百度相当 | 现在处于发展阶段，网络节点种类单一，特别是`海外地区效果可能不佳` |`20元/月` 期间`无限调用次数`  | 高 | 一般在0.8s左右，可自由选择节点 | ✔️ | ✔️ |一般在0.8s左右，可自由选择节点 | ✔️ | ✔️ |
