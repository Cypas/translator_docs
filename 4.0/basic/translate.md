...
14. 之后需要配置代理地址，在这方面翻译器不提供相关教程，只教如何填写，若使用的软件是`clash`，这里填写`127.0.0.1:7890`，其他软件填写方法类似
    - ![填入配置代理](../assets/img/203.webp ':size=50%')
15. 模型的设置与api接口地址
    - 模型现在的下拉框没有gpt4的选项，如果api有gpt4使用权限且希望在翻译器内使用，需要自行更改模型名称，下拉框这里是可以进行编辑内容的，将默认的`gpt-3.5-turbo-0613`改为`gpt-4-0312`，就可以使用gpt4了
      - ![gpt4](../assets/img/205.webp ':size=50%')
    - api接口地址默认为官方接口地址`https://api.openai.com/v1/chat/completions` ，一般不需要更改此项，如果有自建代理源，才需要更改，另外，这里需要填写完整的接口地址，例如商家只提供了一个域名`https://xxx.com`，后面需要自己接上完整的接口地址，如`https://xxx.com/v1/chat/completions`
      - ![gpt4](../assets/img/204.webp ':size=50%')
16. 点击测试查看翻译是否可用，若不可用，请检查代理或代理地址是否可用
