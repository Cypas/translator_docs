...
## ocr错误:团子翻译器未启动或运行失败
- 如果是选择的离线ocr模式（默认也就是这种模式），需要先开启离线ocr且最小化其窗口，不要关闭！！！
- 离线ocr使用参考[离线ocr说明](/basic/ocr#离线ocr说明)

## ocr错误:response无响应
- 百度ocr免费额度达到上限，建议换为免费的离线ocr，使用参考[离线ocr说明](/basic/ocr#离线ocr说明)
...
## 竖排文字识别无法生效
- 离线ocr无法实现竖排文字，这个需要百度在线ocr，`设置-其他设定`内的`竖排文字`以及`高精度模式`都是只有百度在线ocr可用
- 百度ocr免费额度参考[百度ocr说明](/basic/ocr#百度ocr说明)

![应用接口](../assets/img/47.webp ':size=50%')

## 由于目标计算机积极拒绝，无法连接
- 你没启动离线ocr程序或把离线ocr程序的窗口关了，那个窗口需要一直开 着，建议最小化，参考[离线ocr说明](/basic/ocr#离线ocr说明)

## ocr连接失败惹
- 百度ocr免费额度使用达到上限了
- 百度ocr免费额度参考[百度ocr说明](/basic/ocr#百度ocr说明)
- 可以换离线ocr试试[离线ocr说明](/basic/ocr#离线ocr说明)
...
- 首次启动离线ocr时中途出现这个提示可以不用去管，等进度条7/7跑完就行了，不影响使用
- 如果在这个提示这里或进度4/7，5/7这里卡了很久（5分钟以上），建议把360，腾讯管家等安全类软件关了再重新运行离线ocr
...
## 离线ocr运行时提示:以一种访问权限不允许的方式做了一个访问套接字的尝试
- 表示电脑的6666端口被占用，离线ocr是通过这个端口传递数据的
...
- 这个代码在ip部分每个人稍有不同
- 这个是离线ocr启动完毕，是正常信息，注意这个窗口不要关闭，最小化就行了，然后去打开翻译器使用
