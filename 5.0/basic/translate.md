...
|[Sakura本地翻译模型](/5.0/basic/translate#Sakura本地翻译模型)|不限次数|⭐⭐⭐|⭐⭐⭐⭐⭐|部署难度较高，需要花较多精力，对显卡要求很高(8G显存及以上)，`只能翻译日文原文`| 
...
10. 这里的`两行字符`就是我们需要的密钥，将得到的两个密钥复制到团子翻译器内`腾讯`的密钥编辑框
    - ![复制密钥](../assets/img/283.webp ':size=70%')
    - ![输入密钥](../assets/img/250.webp ':size=50%')

11. 随后点击`测试`按钮，看能否正常使用，能正常使用后关闭密钥页面，并打开腾讯翻译的开关
    - ![输入密钥](../assets/img/251.webp ':size=50%')
...
- 仍有需求的可以按以下教程进行注册，以下教程是官方gpt api接口,但翻译器也是支持第三方gpt接口，或其他符合openai协议的模型接口的，如去某宝搜索`gpt中转api`等关键词也能搜到第三方接口和密钥，最后将店主给的地址与密钥填入到翻译器的gpt设置内即可使用
...
13. 将复制的密钥填入到翻译器ChatGPT密钥页面
    - ![填入密钥](../assets/img/289.webp ':size=50%')
...
## **Sakura本地翻译模型**
## Sakura本地翻译模型说明
!> 本翻译源需要较强电脑方面的动手能力，需要至少8G显存以上的显卡(20系显卡及以上)，`不推荐所有人使用`，另外Sakura是纯日文模型，`只能用来翻译日文原文`，英文是不支持的
